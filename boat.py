...
class Boat:
    ...
    def calculateWrenches(self, vel_field, vel_world_frame, points_world, normals_world_frame, points_canoe_frame, H_canoe):
        ...
        for i in range(points_world.shape[0]):
            ...
            # vdotn = v_world[0]*normals_world_frame[i,0]+v_world[1]*normals_world_frame[i,1]
            # f_world = np.sign(vdotn) * vdotn**2 * normals_world_frame[i]
...
def propelVelField(vel_field, boat):
    ...
    for object_frame in boat.all_effective_frames:
        ...
        for i in range(len(object_points_world_frame)):
            ...
            # v = point_vel - np.array([vel_field.u[int(point[0]), int(point[1])], vel_field.v[int(point[0]), int(point[1])]])
            # n = object_normals_world_frame[i]
            # vdotn = (point_vel[0] - vel_field.u[int(point[0]), int(point[1])])*object_normals_world_frame[i,0] + (point_vel[1] - vel_field.v[int(point[0]), int(point[1])])*object_normals_world_frame[i,1]
            # force = np.sign(vdotn) * vdotn**2 * object_normals_world_frame[i]
