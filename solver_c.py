...
def advect(N, b, d, d0, follow_vel, dt):
    ...
    dt0 = dt * N
    indices = np.indices(d.shape).astype(float64)
    indices[0, 1:-1, 1:-1] -= dt0 * follow_vel.u[1:-1, 1:-1]
    indices[1, 1:-1, 1:-1] -= dt0 * follow_vel.v[1:-1, 1:-1]
    ind = np.clip(indices, 0.5, N+0.5)
    d[1:-1, 1:-1] = bilinear_interp_matrix(d0, ind[0], ind[1])[1:-1, 1:-1]
    # d[1:-1, 1:-1] = d0[ind[0], ind[1]][1:-1, 1:-1]
    # for i in range(1, N + 1):
